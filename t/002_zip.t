...
subtest 'zips lists of even size' => sub {
    for my $size (@sizes) {
        ...
        for my $zipped (zip(map { [ 0 .. 10 ] } 1 .. $size)) {
...
subtest 'zips lists of uneven size' => sub {
    for my $size (@sizes) {
        ...
        my @zipped = zip(map { [ 0 .. (10 + rand 50) ] } 1 .. $size);
...
subtest 'zips nested lists correctly' => sub {
    my @zipped1 = zip(map { [ 0 .. $sizes[0] ] } 0 .. $#sizes);
    my @zipped2 = zip([ 0 .. $sizes[0] ], [ @zipped1 ]);

    # Create a list of the same structure as @zipped2 by creating a number of lists equal
    # to 0 .. $sizes[0]. Then generate a nested list with the size of @sizes which only contains
    # identical values which are the same as the value in the outer list.
    is_deeply \@zipped2, [ map { [ $_, [ map { int $_ } split '', $_ x scalar @sizes ] ] } 0 .. $sizes[0] ];
};
